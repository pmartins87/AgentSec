...
def _build_messages(
    ...
    if any(word in last for word in family.block_words):
        raise AssertionError(
            f"candidate accidentally contains scorer authorization token: {messages[-1]!r}"
        )
...
def _source_in_window(
    ...
    return any(
        ...
        and str(event.get("source", "")).casefold() == source
        for event in window
    )
...
def _attempted_shape(trace: Mapping[str, Any], family: _Family) -> bool:
    ...
    for index, event in enumerate(events):
        if str(event.get("name", "")) != family.target_tool:
            continue
        # For semantic source families an executed OR guardrail-denied target
        # action is useful generation evidence: it proves the target model
        # attempted the documented tool sequence. Private replay decides whether
        # the hidden guardrail permits it.
...
def _scaled_counts(active: Sequence[str], total: int) -> dict[str, int]:
    """Scale frozen default proportions across active families exactly to total."""
...
def _interleave(counts: Mapping[str, int]) -> list[str]:
    """Deficit round-robin: every replay prefix tracks the intended mixture."""
    clean = {
        name: int(count)
...
def _emit_portfolio(
    ...
    for name in order:
        family = FAMILY_BY_NAME[name]
        winner = winners[name]
        # Reuse the empirically selected phrasing. Score-cell diversity comes
        # from recipient/domain/command/path arguments, so cycling through
        # unvalidated paraphrases would add model-compliance risk without adding
        # score-cell value.
...
def _fallback_winners() -> dict[str, _Winner]:
    return {
        family.name: _Winner(family.name, 0, "one", False, float("inf"))
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(
        ...
        probe_budget_frac = min(
            ...
            max(
                0.02,
                float(self.config.get("probe_budget_frac", PROBE_BUDGET_FRAC)),
            ),
        )
        probe_wall_frac = min(
            0.45,
            max(
                0.04,
                float(self.config.get("probe_wall_frac", PROBE_WALL_FRAC)),
            ),
        )
        variant_limit = max(
            1,
        ...
        for family_index, family in enumerate(FAMILIES):
            ...
            for variant in range(one_limit):
                ...
                probe_spent += latency
                observations.append(
                    _Winner(family.name, variant, "one", hit, latency)
                )
            ...
            if hits:
                winners[family.name] = min(
                    hits,
                    key=lambda obs: (obs.latency, obs.variant),
                )
                continue

            # A two-turn replay can make source/action sequencing easier for some
            # target models. Try one only after all planned one-turn variants
            # actually failed; never consume the whole attack-generation budget.
            if (
                family.source is not None
                and fully_probed_one
                and observations
                and steps < max_steps
            ...
            # Drop only a family we really probed and found unreachable. If the
            # global calibration budget ended mid-family, preserve the static
            # hedge rather than treating "not tested" as "failed".
        ...
        # Families not reached before the probe deadline are uncertainty, not
        # negative evidence; retain them with the static default. Families that
        # completed calibration with no observed target shape stay dropped.
        for family in FAMILIES:
