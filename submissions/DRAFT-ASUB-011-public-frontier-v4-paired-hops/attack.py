# ruff: noqa: E402, I001
"""AgentSec DRAFT-ASUB-011 public-frontier v4 paired-hop calibration.
...
Unlike fixed local timing multipliers, this draft estimates the full-hop/one-hop
cost ratio inside each live hosted generation call. It only enables one-hop fill
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(
        ...
        # Generation-only warm-up. Replay gets a fresh evaluator deadline.
        try:
            env.reset()
        ...
        # First full-hop sample establishes a replay-realistic row speed. If the
        # plain form misses, try Harmony once before abandoning live adaptation.
        ...
        # Slow rows prefer Harmony if it was the only successful classifier; a
        # successful plain slow classifier keeps plain here so the hop-depth test
        # isolates latency accounting rather than adding another large A/B branch.
        ...
        # Paired live hop calibration. Full-hop positives are charged at measured
        # replay cost; one-hop positives are banked only after a scale is known.
        ...
        # If paired calibration is insufficient, do not return one-hop-only
        ...
        def fill_segment(replay_cap: float, wall_deadline: float, segment: int) -> None:
            ...
            while len(verified) < hard_cap and steps < max_steps:
                ...
                # Project replay at full-hop cost. For full-hop fill, scale=1.
