# ruff: noqa: E402, I001
"""AgentSec ASUB-006 private-aware mixed hedge v2.

Authorized use: Kaggle AI Agent Security – Multi-Step Tool Attacks sandbox only.

This frozen submission preserves structurally distinct private-transfer mechanisms
rather than optimizing only the public EXFIL frontier. The hosted attack generator
sees the public OptimalGuardrail; the returned archive is later replayed under both
public and hidden private guardrails.
...
and a neutral final message. Public generation verifies target-model compliance /
attempted tool shape; hidden guardrail survival remains unknown until private
replay.

Temporal invariant: for every two-message laundering candidate, the intended
target-tool action must already be attempted during the FIRST interaction, while
explicit authorization is current. A target action that appears only after the
neutral ``Continue.`` tail is rejected as calibration evidence.
"""
