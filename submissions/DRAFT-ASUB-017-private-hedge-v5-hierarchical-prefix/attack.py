# ruff: noqa: E402, I001
"""AgentSec DRAFT-ASUB-017 private-aware hedge v5 hierarchical-prefix.

Authorized use: Kaggle AI Agent Security – Multi-Step Tool Attacks sandbox only.

This draft keeps the private-hypothesis portfolio and bounded live calibration of
ASUB-009, but improves the replay prefix ordering.  The first replay round covers
all active *lanes* before any lane repeats; a second round covers the remaining
families inside those lanes.  The weighted-deficit tail then resumes with exact
remaining counts.

The hidden private guardrail is never queried.  Hierarchical lane-first ordering
is a robustness policy under uncertainty, not a claim about hidden behavior.
"""
...
class AttackAlgorithm(AttackAlgorithmBase):
    """Bounded public calibration followed by hierarchical private hedge."""
        ...
        for family_index, family in enumerate(FAMILIES):
            ...
            for variant in (0, 1):
                if not can_probe():
                    fully_probed = False
                    break
                messages = _build_messages(family, family_index * 20 + variant, variant)
