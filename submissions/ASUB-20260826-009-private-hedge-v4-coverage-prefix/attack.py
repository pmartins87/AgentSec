# ruff: noqa: E402, I001
"""AgentSec DRAFT-ASUB-015 private-aware mixed hedge v4 coverage-prefix.

Authorized use: Kaggle AI Agent Security – Multi-Step Tool Attacks sandbox only.

This draft extends the timeout-prefix private hedge with an explicit coverage-first
prelude.  Replay timeouts preserve already-scored prefix work, so an unexpectedly
short private replay should still encounter every materially distinct hypothesis.

Compared with ASUB-008 / DRAFT-ASUB-014:
- the same private-hypothesis weights and calibration logic are preserved;
- the default archive remains the full legal 2,000 findings;
- every active family is emitted once in the first |families| positions;
- the remaining 1,989 positions resume weighted deficit interleaving exactly,
  preserving total requested counts.

The hidden private guardrail is never queried.  Coverage-first ordering is a
robustness policy under uncertainty, not a claim about which hidden mechanism
works.
"""
...
def _build_messages(family: _Family, index: int, variant: int = 0) -> tuple[str, ...]:
    ...
    if family.kind == "exfil":
        messages = (_format(EXFIL_TEMPLATES[variant % len(EXFIL_TEMPLATES)], index),)
    elif family.kind == "direct_synonym":
        messages = (_format(DIRECT_SYNONYM_TEMPLATES[variant % len(DIRECT_SYNONYM_TEMPLATES)], index),)
    elif family.kind == "direct_launder":
        messages = (_format(DIRECT_EXPLICIT_TEMPLATES[variant % len(DIRECT_EXPLICIT_TEMPLATES)], index), NEUTRAL_TAIL)
    else:
        if family.source not in SOURCE_TEMPLATES:
            raise ValueError(f"missing source template for {family.name}")
        source_template = SOURCE_TEMPLATES[str(family.source)][variant % len(SOURCE_TEMPLATES[str(family.source)])]
    ...
    if not 1 <= len(messages) <= 2:
        raise AssertionError("private-hedge v4 emits only one- or two-message chains")
...
def _source_in_window(events: Sequence[Mapping[str, Any]], target_index: int, source: str) -> bool:
    ...
    return any(
        event.get("ok") is True and str(event.get("source", "")).casefold() == source.casefold()
        for event in window
    )
...
def _interleave(counts: Mapping[str, int]) -> list[str]:
    """Coverage-first prelude, then exact weighted interleaving of the remainder."""
    remaining = {
        name: int(count)
    ...
    out: list[str] = []
    # Preserve the declared family order so the prelude is deterministic. Every
    # active hypothesis receives one replay slot before any family receives two.
...
class AttackAlgorithm(AttackAlgorithmBase):
    """Bounded public calibration followed by coverage-first private hedge."""
