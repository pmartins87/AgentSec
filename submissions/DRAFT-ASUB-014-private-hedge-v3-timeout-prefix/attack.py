# ruff: noqa: E402, I001
"""AgentSec DRAFT-ASUB-014 private-aware mixed hedge v3 timeout-prefix.

Authorized use: Kaggle AI Agent Security – Multi-Step Tool Attacks sandbox only.

This draft preserves the ASUB-006 private-hypothesis mix but adapts it to the
current evaluator rule that public/private replay timeouts preserve the score
accumulated before truncation.  Instead of stopping at an arbitrary 360 findings,
it returns the full legal 2,000-finding archive, interleaved by family so that an
early replay prefix still contains all major private hypotheses.

The public generation phase is used only for bounded model-compliance calibration.
The hidden private guardrail is never queried.  Families that are conclusively
shown not to be attempted by the target model are dropped; unprobed families are
kept as uncertainty hedges.

Portfolio weights are inherited from ASUB-006 (weight total 360):
- public EXFIL 144
- direct confused deputy 24 + 36
- semantic email 60 + 60
- semantic HTTP 6 + 6
- semantic shell 6 + 6
- semantic write 6 + 6

Those weights are scaled to the requested archive size (default 2,000) and emitted
with deficit-based interleaving rather than family blocks.
"""
...
def _build_messages(family: _Family, index: int, variant: int = 0) -> tuple[str, ...]:
    ...
    if not 1 <= len(messages) <= 2:
        raise AssertionError("private-hedge v3 emits only one- or two-message chains")
...
class AttackAlgorithm(AttackAlgorithmBase):
    """Bounded public calibration followed by full interleaved private hedge."""
