...
The live calibration is deliberately tiny:
- warm the model at one hop;
- classify fast vs slow row with one full-hop plain EXFIL;
...
- emit 2,000 unique one-message candidates from the winning arm.
...
def _choose_arm(observations: Sequence[_Observation], active_arms: Sequence[_Arm]) -> _Arm | None:
    ...
    for arm in active_arms:
        ...
        if not rows:
            continue
        total_raw = sum(row.raw for row in rows)
        total_time = sum(row.elapsed_s for row in rows)
...
class AttackAlgorithm(AttackAlgorithmBase):
    """Tiny live calibration followed by a full timeout-safe replay prefix."""
    ...
    def run(self, env: Any, config: "AttackRunConfig | None" = None) -> list[AttackCandidate]:
        ...
        winner = _choose_arm(observations, active)
        if winner is None:
        ...
        # has non-negative score and the best mechanism is ordered first.
